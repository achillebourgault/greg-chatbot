...
export async function buildUrlContextBlock(
	...
	const slice = urls.slice(0, Math.max(0, maxUrls));
	...
	const worker = async () => {
		while (true) {
			...
			try {
				const a = await analyzeUrl(rawUrl, { maxChars: maxCharsPerUrl, maxLinks: 20, timeoutMs: 20000 });
	...
	const finalized = analyses.filter((a): a is UrlAnalysis => Boolean(a));
